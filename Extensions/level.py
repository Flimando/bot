...
class Level(commands.Cog):
    ...
    async def setup_level(self, interaction: discord.Interaction, announcement_channel: discord.TextChannel = None):
        """Richtet das Level-System in einem Server ein"""
        guild_id = str(interaction.guild_id)
        
        if guild_id not in ALLOWED_GUILDS.get("level", []):
            embed = discord.Embed(
                title="❌ Fehler",
                description="Das Level-System ist in diesem Server nicht verfügbar!",
                color=COLORS["red"]
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        
        if setup_level_system(guild_id):
            # Setze Announcement-Channel wenn angegeben
            if announcement_channel:
                set_announcement_channel(guild_id, str(announcement_channel.id))
                channel_info = f"\n• **Level-Up Channel:** {announcement_channel.mention}"
            else:
                channel_info = "\n• **Level-Up Channel:** Nicht gesetzt (kann später mit `/change-announcement` gesetzt werden)"
            
            embed = discord.Embed(
                title="✅ Level-System aktiviert!",
                description="Das Level-System wurde erfolgreich eingerichtet!" + channel_info + "\n\n**Verfügbare Commands:**\n• `/level` - Dein Level anzeigen\n• `/leaderboard` - Server-Rangliste\n• `/rank @user` - Level von anderen\n• `/change-announcement` - Level-Up Channel ändern\n• `/show-announcement` - Aktuellen Level-Up Channel anzeigen\n• `/block-channel` - Einzelnen Channel blockieren\n• `/block-channels` - Mehrere Channels blockieren\n• `/unblock-channel` - Einzelnen Channel freigeben\n• `/unblock-channels` - Mehrere Channels freigeben\n• `/block-all-except` - Alle außer bestimmte blockieren\n• `/list-blocked` - Blockierte Channels anzeigen",
                color=COLORS["green"]
            )
        else:
            embed = discord.Embed(
                title="ℹ️ Level-System bereits aktiv",
                description="Das Level-System ist bereits für diesen Server eingerichtet!",
                color=COLORS["blue"]
            )
    ...
    async def change_announcement(self, interaction: discord.Interaction, channel: discord.TextChannel):
        """Ändert den Announcement-Channel für Level-Ups"""
        guild_id = str(interaction.guild_id)
        ...
        if level_up:
            ...
            if announcement_channel_id:
                try:
                    ...
                    # Fallback: Sende in aktuellen Channel
                    await message.channel.send(embed=embed)
            else:
                # Sende in aktuellen Channel wenn kein Announcement-Channel gesetzt
                await message.channel.send(embed=embed)
